...
using Library.Management.System.Repository.Specifications.BookSpecs;
...
{  
    ...
    public class BooksController : BaseApiController
    {
        ...
        /*========================= EndPoints Using Dynamic  Query Using Specification Design Pattern=====================*/
        ...
        public async Task<ActionResult<Pagination<BookDto>>> GetALLBooksUsingSpec([FromQuery] BookParams bookParams)
        {
            try
            {
                int Count;
                ...
                if (!_cache.TryGetValue(cacheKey, out IReadOnlyList<BookDto> mappedBooksIntoBooksDto))
                {
                    var spec = new BookSpecifications( bookParams);
        ...
        [Authorize]
        [HttpGet("WithSpec/{id}")]
        public async Task<ActionResult<BookDto>> GetBookByIdUsingSpec(int id)
        {
            var spec = new BookSpecifications(id);
            var book = await _unitOfWork.Repository<Book>().GetWithSpecAsync(spec);
            if (book == null)
                return NotFound(new { Message = $"There is no any book with this id {id}" });
            // Map book to BookDto
            var mappedBookIntoBookDto = _mapper.Map<Book, BookDto>(book);
